    ...
    <section id={id} className={`${isDark ? "section-dark flow-section-dark" : "section-cream flow-section-cream"} py-24 relative`}>
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 relative z-[1]">
        <div className="grid lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <ul className="space-y-2">
                {goals.map((g) => (
                  <li key={g} className={`text-sm flex items-start gap-2 ${isDark ? "text-white" : "text-secondary-foreground/70"}`}>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {formats.map((f) => (
                  <div key={f} className={`flex items-center gap-2.5 text-sm ${isDark ? "text-white" : "text-secondary-foreground/70"}`}>
          ...
          <div className="flex items-center justify-center h-full self-stretch">
            {phoneImage}
          </div>
        </div>
      </div>
    ...
    <div className="metric-card flow-corner-br">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-white/60 mt-1">Monthly page ranking distribution across search result pages.</p>
        </div>
      ...
      <p className="text-[10px] text-white/40 mt-2 text-center">Scroll to zoom · Drag to select range</p>
    </div>
  );
}
...
export function NorthAmericaExtra() {
  return (
    <div className="metric-card grid grid-cols-3 gap-3">
      <p className="text-white text-sm col-span-3">No additional data for this section.</p>
    </div>
  );
}
    ...
    <div className="glass-card-dark flow-corner-br border border-foreground/10">
      ...
      <ul className="space-y-3">
        {learnings.map((l: string) => (
          <li key={l} className="text-sm text-white flex items-start gap-3">
